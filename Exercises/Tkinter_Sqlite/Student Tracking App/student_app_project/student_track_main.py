...
class ParentWindow(Frame): 
    def __init__(self, master, *args, **kwargs):
        ...
        self.master = master
        self.master.minsize(500,400) #(Height, Width pixels)
        self.master.maxsize(500,400)
        # This CenterWindow method will center our app on the user's screen
        student_track_func.center_window(self,500,400) # have to pass in self, it is the place we are working in and referring to
        self.master.title("Student Tracker App")
        self.master.configure(bg="#F0F0F0")
        # This protocol method is a tkinter built-in method to catch if 
        # the user clicks the upper corner, "X" on Windows OS.
        self.master.protocol("WM_DELETE_WINDOW", lambda: student_track_func.ask_quit(self))
        arg = self.master

        # load in the GUI widgets from a separate module, 
        # keeping your code comparmentalized and clutter free
        student_track_gui.load_gui(self) # have to pass in self
        ...
        filemenu.add_command(label="Exit", underline=1,accelerator="Ctrl+Q",command=lambda: student_track_func.ask_quit(self))
