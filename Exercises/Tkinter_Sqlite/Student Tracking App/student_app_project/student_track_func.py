# Python Ver:   3.10.9
#
# Author:       Blended Instructor and Me
#
# Purpose:      Student Tracker App
#
# 
...
import student_track_main
import student_track_gui



def center_window(self, w, h): # pass in the tkinter frame (master) reference and the w and h
    # get user's screen width and height
    screen_width = self.master.winfo_screenwidth()      # tkinter's function for getting user's screen info - winfo_screenwidth()
    screen_height = self.master.winfo_screenheight()
    # calculate x and y coordinates to paint the app centered on the user's screen
    # very neat little set of calculations to find the center!
...
def ask_quit(self):
    if messagebox.askokcancel("Exit program", "Okay to exit application?"):
        # This closes app
        self.master.destroy()
        os._exit(0)    # function of os. makes sure all widgets are closed and deletes from memory, releases memory, so there are no memory leaks.


#=========================================================
def create_db(self):
    conn = sqlite3.connect('student_tracker.db')
    with conn:  # = if successful
        cur = conn.cursor()
        cur.execute("CREATE TABLE if not exists tbl_student_trk( \
...
def first_run(self):
    ...
    with conn:  # = if successful
        ...
        if count < 1:
            cur.execute("INSERT INTO tbl_student_trk (col_fname,col_lname,col_fullname,col_phone,col_email,col_course) VALUES (?,?,?,?,?,?)", ('Jane','Eastman','Jane Eastman','123-456-7890','jeastman@email.com','software dev')) # removed extra """
            conn.commit()
    conn.close() 
...
def count_records(cur):
    count = ""
    cur.execute("""SELECT COUNT(*) FROM tbl_student_trk""")
...
def onSelect(self,event):
    ...
    with conn:  # = if successful
        cursor = conn.cursor()
        cursor.execute("""SELECT col_fname,col_lname,col_phone,col_email,col_course FROM tbl_student_trk WHERE col_fullname = (?)""", [value])
...
def addToList(self):
    ...
    if (len(var_fname) > 0) and (len(var_lname) > 0) and (len(var_phone) > 0) and (len(var_email) > 0 ) and (len(var_course) > 0 ): # enforce the user to provide both names
        ...
        with conn:
            ...
            cursor.execute("""SELECT COUNT(col_fullname) FROM tbl_student_trk WHERE col_fullname = '{}'""".format(var_fullname))#,(var_fullname))
            count = cursor.fetchone()[0]
            chkName = count
            if chkName == 0: # if this is 0 then there is no existance of the fullname and we can add new data
                print("chkName: {}".format(chkName))
                cursor.execute("""INSERT INTO tbl_student_trk (col_fname,col_lname,col_fullname,col_phone,col_email,col_course) VALUES (?,?,?,?,?,?)""",(var_fname,var_lname,var_fullname,var_phone,var_email,var_course))
...
def onDelete(self):
    var_select = self.lstList1.get(self.lstList1.curselection()) # Listbox's selected value - get is built in function
    conn = sqlite3.connect('student_tracker.db')
    with conn:
        cur = conn.cursor()
        ...
        if count > 1:
            ...
            if confirm:
                ...
                with conn:
                    cursor = conn.cursor()
                    cursor.execute("""DELETE FROM tbl_student_trk WHERE col_fullname = '{}'""".format(var_select))
...
def onRefresh(self):
    ...
    with conn:
        cursor = conn.cursor()
        cursor.execute("""SELECT COUNT(*) FROM tbl_student_trk""")
        count = cursor.fetchone()[0]
        i = 0
        while i < count:
                cursor.execute("""SELECT col_fullname FROM tbl_student_trk""")
...
def onUpdate(self):
    ...
    if (len(var_phone) > 0) and (len(var_email) > 0) and (len(var_course) > 0): # ensure that there is data present
        ...
        with conn:
            ...
            cur.execute("""SELECT COUNT(col_phone) FROM tbl_student_trk WHERE col_phone = '{}'""".format(var_phone))
            count = cur.fetchone()[0]
            print(count)
            cur.execute("""SELECT COUNT(col_email) FROM tbl_student_trk WHERE col_email = '{}'""".format(var_email))
            count2 = cur.fetchone()[0]
            print(count2)
            cur.execute("""SELECT COUNT(col_course) FROM tbl_student_trk WHERE col_course = '{}'""".format(var_course))
            ...
            if count == 0 or count2 == 0 or count3 == 0: # if proposed changes are not already in the database, then proceed
                ...
                if response:
                    ...
                    with conn:
                        cursor = conn.cursor()
                        cursor.execute("""UPDATE tbl_student_trk SET col_phone = '{0}',col_email = '{1}',col_course = '{2}' WHERE col_fullname = '{3}'""".format(var_phone,var_email,var_course,var_value))
                        onClear(self)
                        conn.commit()
                else:
                    messagebox.showinfo("Cancel request","No changes have been made to ({}).".format(var_value))
            else:
                messagebox.showinfo("No changes detected","Both ({}) and ({}) and ({}) \nalready exist in the database for this name. \n\nYour update request has been cancelled.".format(var_phone, var_email, var_course))
            onClear(self)
        conn.close()
    else:
