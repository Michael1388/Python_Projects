...
def balance(request, pk):
    ...
    for t in transactions:  # Loop through transactions and determine which is a deposit ir withdrawal
        if t.type == 'Deposit':
...
def transaction(request):
    ...
    # Checks if request method is POST
    if request.method == 'POST':
        if form.is_valid():   # Check if submitted form is valid and if so, saves the form
